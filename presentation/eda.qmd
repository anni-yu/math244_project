...
# Save the figure
ggsave("figures/plot1_wage_distribution_complete_cases.png", width = 8, height = 5, dpi = 300)
```
...
ggplot(wage_data_imputed, aes(x = wage_percent_of_male)) +
  ...
  labs(
    ...
    x = "Female Earnings as % of Male Earnings",
    y = "Density",
    caption = "Source: BLS via TidyTuesday | 2013–2016 | Includes all occupations"
  ) +
  theme_minimal(base_size = 13)
...
# Save the figure
ggsave("figures/plot2_wage_distribution_imputed.png", width = 8, height = 5, dpi = 300)
```
...
wage_data_complete_cases %>%
  ...
  labs(
    title    = "Median Female-to-Male Wage Ratio by Industry Sector (2013–2016)",
    subtitle = "Dashed line = wage parity (100%)",
    x        = "Year",
    y        = "Median Female Earnings as % of Male",
    color    = "Industry Sector",
    caption  = "Source: BLS via TidyTuesday"
  ) +
...
# Save the figure
ggsave("figures/plot3_trend_by_sector.png", width = 10, height = 6, dpi = 300)
```
